function validation({
  ...
  if (nacionality.length < 5)
    errors.nationality = "Must be at least 5 characters";
  ...
  if (!onlyLetters.test(nacionality))
    errors.nationality = "numbers are not allowed";
  if (!url.test(image) || !imageurl.test(image))
    errors.image = "must be the URL of an image";
